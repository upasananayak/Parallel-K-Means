...
vector<Point> readcsv(const string &filename) {
    ...
    while (getline(file, line)) {
        ...
        // Read the two columns from the file
        getline(lineStream, bit, ',');  // Get the first column (x coordinate)
        x = stof(bit);
        getline(lineStream, bit, ',');  // Get the second column (y coordinate)
        y = stof(bit);

        // Push the point into the points vector
...
int main() {
    vector<Point> points = readcsv("data_20k.csv");

    auto start = chrono::high_resolution_clock::now(); // Start time

    // Run k-means with 200 iterations and for 20 clusters
    kMeansClustering(&points, 200, 20);
