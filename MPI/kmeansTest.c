...
void save(data_struct* data2save, char *filename1, char *filename2){
    ...
    printf("Saving data to files: %s and %s\n", filename1, filename2);
    ...
    for(i = 0; i < m; i++){
        for(j = 0; j < n; j++){
            fprintf(outfile, "%f ", tmp_dataset[i*n + j]);
        }
        fprintf(outfile, "\n");
    }
    fclose(outfile);
...
int main(int argc, char **argv){
  ...
  char *file1_0 = "centroidsP.txt";
    char *file1_1 = "ClusterSizeP.txt";
    char *file2_0 = "datasetP.txt";
    char *file2_1 = "IndexP.txt";
  data_struct data_in;
 ...
 // printf("Process %i will take %ld number of objects.\n", rank, numObjects);
  ...
  printf("Time elapsed: %ld.%06ldsec\n", lapsed.tv_sec, lapsed.tv_usec);
